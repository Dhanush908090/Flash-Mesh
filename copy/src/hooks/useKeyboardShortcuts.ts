...
export function useKeyboardShortcuts({
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    if (ctrl && e.key === 'u') { e.preventDefault(); document.querySelector<HTMLElement>('#upload-btn')?.click(); return; }
    if (ctrl && e.key === 'd') { e.preventDefault(); document.querySelector<HTMLElement>('#download-btn')?.click(); return; }
    if (ctrl && shift && e.key === 'N') { e.preventDefault(); onNewFolder(); return; }

    // ── New File (Ctrl+N without shift)
    ...
    // ── Open selected item with Enter
    if (e.key === 'Enter' && selectedEntries.length > 0) {
      e.preventDefault();
    ...
    if (e.key === 'Escape') {
      ...
      onCopy, onCut, onPaste, onDelete, onRename, onNewFolder, onNewFile, onOpen,
      onSelectAll, onRefresh, onOpenSpotlight, onUndo, onRedo]);
