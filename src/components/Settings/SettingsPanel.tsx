...
import { X, Settings as SettingsIcon, Command, Info, Cloud, MessageSquare, Send, ExternalLink, CheckCircle, AlertCircle, Loader, KeyRound, ChevronDown, ChevronUp, Circle, Box } from 'lucide-react';
...
function ProviderCard({ provider, label, accentColor, isConnected, quota, onConnect, onDisconnect }: ProviderCardProps) {
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: isConnected && quota ? 14 : (authState !== 'idle' ? 14 : 0) }}>
        <div style={{
          ...
          boxShadow: `inset 0 1px 0 rgba(255,255,255,.24), 0 12px 24px ${accentColor}18`,
          color: accentColor,
        }}>
          {provider === 'google' ? <Circle size={18} fill="currentColor" /> : <Box size={20} />}
        </div>
        <div style={{ flex: 1 }}>
    ...
    <>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        >
          <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <KeyRound size={14} /> <span>Developer API Credentials</span>
          </span>
      ...
      <div style={{ padding: 16, borderRadius: 10, background: 'rgba(59,130,246,.06)', border: '1px solid rgba(59,130,246,.15)' }}>
        ...
        <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 8, marginBottom: 0, opacity: 0.65 }}>
          Auth opens in your system browser. FlashMesh auto-connects once you sign in; no copy-pasting required.
        </p>
      </div>
    </>
  );
  ...
  return (
    <div className="dialog-backdrop" onClick={onClose}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 24px', borderBottom: '1px solid var(--border)' }}>
          <div className="dialog__title" style={{ marginBottom: 0 }}>Settings</div>
              ...
              <>
                ...
                <SettingsRow label="Encryption Strength" description="Select the security policy for uploaded chunks">
                  <select
                    ...
                    className="settings-select"
                  >
                    <option value="adaptive">Adaptive (Headers Encrypted, 25% raw for speed)</option>
