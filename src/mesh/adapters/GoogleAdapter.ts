...
export class GoogleAdapter implements CloudAdapter {
  ...
  private clientId: string = 'YOUR_GOOGLE_CLIENT_ID'; // Replaced at runtime

  get isAuthenticated(): boolean {
    return this.accessToken !== null;
  }

  // Simplified OAuth flow modeled from old basic build
  async authenticate(): Promise<string | null> {
    return new Promise((resolve) => {
      ...
      this.clientId = '376295481193-2gr1nfv4ef1u7dpqv865idj9p6m7m5ot.apps.googleusercontent.com';
  ...
  async listFolder(path: string): Promise<any[]> {
    ...
    const res = await fetch(`https://www.googleapis.com/drive/v3/files?q=${query}&fields=files(id,name,size,mimeType,modifiedTime)`, {
      headers: { Authorization: `Bearer ${this.accessToken}` }
    });
