...
export async function executePaste(
  ...
  try {
    switch (strategy) {
      ...
      case 'pool-transfer': {
        onProgress?.(10, 'Initiating pool transfer...');
        const isDestPool = destPath.startsWith('pool://');

        if (isDestPool) {
          const parts = destPath.substring('pool://'.length).split('/');
          ...
          for (let i = 0; i < files.length; i++) {
            const f = files[i];
            const percent = 10 + Math.floor((i / files.length) * 80);
            onProgress?.(percent, `Uploading ${f.name} to pool...`);

            let blob: Blob;
            if (f.provider === 'local') {
              const bytes = await opsApi.readBinaryFile(f.path);
              blob = new Blob([bytes]);
            } else if (f.provider === 'google' || f.provider === 'dropbox') {
              const adapter = f.provider === 'google' ? googleAdapter : dropboxAdapter;
              const manifestJson = await downloadManifest(adapter, `${f.name}.oro`);
              const manifest = JSON.parse(manifestJson);
              blob = await meshEngine.downloadFile(manifest, defaultKeyMaterial);
            } else if (f.provider === 'pool') {
              const srcParts = f.path.substring('pool://'.length).split('/');
              const srcPoolId = srcParts[0];
              const srcFileId = srcParts[1];
              blob = await dataPoolManager.downloadFromPool(srcPoolId, srcFileId);
            } else {
              throw new Error(`Unsupported source provider: ${f.provider}`);
            }

            const jsFile = new File([blob], f.name);
            await dataPoolManager.uploadToPool(poolId, jsFile, folderId);
            ...
            if (operation === 'cut') {
              if (f.provider === 'local') {
                ...
                const srcPoolId = srcParts[0];
                const srcFileId = srcParts[1];
                await dataPoolManager.deleteFile(srcPoolId, srcFileId);
              }
            }
          }
        } else {
          for (let i = 0; i < files.length; i++) {
            ...
            if (f.provider !== 'pool') {
              throw new Error(`Expected source provider to be pool, got: ${f.provider}`);
            }
            ...
            const blob = await dataPoolManager.downloadFromPool(srcPoolId, srcFileId);

            if (destPath.startsWith('mesh://') || destPath.startsWith('google://') || destPath.startsWith('dropbox://') || destPath === 'mesh://root') {
              const jsFile = new File([blob], f.name);
              await meshEngine.uploadFile(jsFile, defaultKeyMaterial);
            } else {
              const arrayBuffer = await blob.arrayBuffer();
